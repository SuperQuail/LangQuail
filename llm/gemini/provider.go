	...
	for _, message := range messages {
		...
		case llm.RoleTool:
			if hasImageInput(message) {
				return nil, nil, fmt.Errorf("llm/gemini: image parts cannot be encoded in tool result messages")
			}
			name := message.Name
			if name == "" {
...
func convertMessageParts(message llm.Message) ([]*genai.Part, error) {
	...
	for _, part := range message.Input {
		...
		default:
			return nil, fmt.Errorf("llm/gemini: unsupported input part type %q", part.Type)
		}
	}
...
func convertImagePart(image *llm.InputImage) (*genai.Part, error) {
	if image == nil {
		return nil, fmt.Errorf("llm/gemini: image input is missing image data")
	}
	if image.URL != "" {
		...
		if image.MIMEType == "" {
			return nil, fmt.Errorf("llm/gemini: image url requires mime type")
		}
		return genai.NewPartFromURI(image.URL, image.MIMEType), nil
	}
	if len(image.Data) == 0 {
		return nil, fmt.Errorf("llm/gemini: image input requires url or data")
	}
	if image.MIMEType == "" {
		return nil, fmt.Errorf("llm/gemini: image data requires mime type")
	}
...
func parseImageDataURL(value string) (string, []byte, bool, error) {
	...
	if comma < 0 {
		return "", nil, true, fmt.Errorf("llm/gemini: invalid image data url")
	}
	metadata := value[len("data:"):comma]
	if !strings.Contains(metadata, ";base64") {
		return "", nil, true, fmt.Errorf("llm/gemini: image data url must be base64 encoded")
	}
	mediaType := strings.Split(metadata, ";")[0]
	if mediaType == "" {
		return "", nil, true, fmt.Errorf("llm/gemini: image data url requires mime type")
	}
	data, err := base64.StdEncoding.DecodeString(value[comma+1:])
	if err != nil {
		return "", nil, true, fmt.Errorf("llm/gemini: decode image data url: %w", err)
	}
